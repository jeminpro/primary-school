...
export function Summary({ total, correct, medianMs, byTable, onTryAgain }: Props) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-6 space-y-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-md">
          ...
          <div className="flex flex-col items-start gap-3">
            ...
            <div className="tooltip w-full" data-tip={`${getRatingRange("accuracy", accuracyRating)}: ${getRatingDescription("accuracy", accuracyRating)}`}>
              <RatingDial value={acc} rating={accuracyRating} width={120} height={8} showRatingText={true} />
            </div>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-md">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-md">
        <div className="font-bold text-xl text-slate-800 mb-6">By Table</div>
        <div className="space-y-8">
          {byTable.map(t => (
            <div key={t.a} className="border-b border-slate-100 pb-6 last:border-b-0 last:pb-0">
              <div className="font-semibold text-lg mb-3 text-violet-700">×{t.a}</div>
              <ul className="space-y-3">
                {t.items.map((it, idx) => (
                  <li 
                    key={`${it.a}x${it.b}-${idx}`} 
                    className="flex items-center justify-between py-2 px-4 rounded-xl bg-slate-50"
                  >
                    <div className="flex items-center gap-3">
                      <span className={`w-6 h-6 flex items-center justify-center rounded-full ${it.correct ? 'bg-green-100' : 'bg-red-100'}`}>
                        {it.correct 
                    ...
                    <span className="inline-flex items-center gap-1 text-slate-500 bg-white px-3 py-1 rounded-full text-sm border border-slate-100">
                      <Timer size={14} />
      ...
      <div className="flex justify-between gap-6 mt-8">
        <button 
          className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-base font-medium py-3 px-6 rounded-full flex items-center gap-2 transition-all duration-200"
        ...
        <button 
          className="bg-violet-600 hover:bg-violet-700 text-white text-base font-medium py-3 px-6 rounded-full flex items-center gap-2 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
          onClick={onTryAgain}
        >
          <RotateCw size={18} />
          Try again
